...
function facebookall_get_fb_comments() {
  ...
  if ($fball_settings['enablecomments'] == '1') {
   ...
   $show_comments .= '<div class="fb-comments" data-href="'. get_permalink() .'" data-width="'.$fball_settings['comment_width'].'" data-num-posts="'.$fball_settings['comment_numpost'].'" data-order-by="'.$fball_settings['comment_order_by'].'" data-colorscheme="'.$colorscheme.'"></div>';
...
function facebookall_get_socialshre() {
  ...
  //Output
    $output .= "<style>.fballshare_left {float:left;}.fballshare { margin: 0px; text-align:center} .fball_fbshare{width:90px;}.fball_fblike{width:100px;}";
  if($layout_style=='1') {
    $output .= ".fball_plusone {width:70px;margin-top:4px;}.fball_twitter {width:90px;margin-top:4px;}";
   } 
   else {
   $output .= ".fball_fblike{width:70px;margin-top:1px;} .fball_fbshare{width:70px;} .fball_twitter {width:66px;}.fball_plusone {width:62px;margin-left:5px}";
   }
   
$output .="</style><div class='fballshare'>";
    if ($fball_settings['share_facebooklike']== '1' ) {
      $output .= "<div class='fballshare_left fball_fbshare'><div class='fb-like' data-href='" . get_permalink() . "' data-layout='" . $like_btn_style . "' data-action='like' data-size='small'></div>
</div>";
    }
  if ($fball_settings['share_facebook']== '1' ) {
      $output .= "<div class='fball_fblike fballshare_left'><div class='fb-share-button' data-href='" . get_permalink() . "' data-layout='" . $like_btn_style . "' data-size='small'></div></div>";
    }
    if ($fball_settings['share_twitter']== '1') {
      $output .= "<div class='fball_twitter fballshare_left'><a href='http://twitter.com/share' class='twitter-share-button' data-text='" . $title. "' data-url='" . get_permalink() . "' data-show-count='true'>Tweet</a></div>";
    }
