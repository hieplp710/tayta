...
function facebookall_make_userlogin() {
	...
	if (isset($_GET['code']) AND !empty($_GET['code']) AND !empty($_GET['state']) AND $_GET['state'] == 'fball_facebook') {
		$code = $_GET['code'];
		if(!empty($code)){
			$get_access_data = facebookall_get_fb_contents("https://graph.facebook.com/v2.8/oauth/access_token?" . 'client_id=' . $fball_settings ['apikey'] . '&redirect_uri=' . urlencode(site_url('/')) .'&client_secret=' .  $fball_settings ['apisecret'] . '&code=' . urlencode($code));
			$access_data = json_decode($get_access_data);
		}
		if(!empty($access_data->access_token)){
			$access_token = $access_data->access_token;
		}
		else{
			echo '<pre>'.$get_access_data.'<br> Follow :- <a href="https://developers.facebook.com/docs/graph-api/using-graph-api/#errorcodes">Receiving Error Codes</a>.';
			exit;
		}
		?>
		<script>
			window.opener.FbAll.parentRedirect({'fball_action' : 'fball', 'fball_access_token' : '<?php echo $access_token?>'});
			window.close();
		</script>
		<?php }
		if(!empty($_REQUEST['fball_access_token']) AND isset($_REQUEST['fball_redirect'])) {
			$fbuser_info = json_decode(facebookall_get_fb_contents("https://graph.facebook.com/v2.8/me?fields=id,name,first_name,last_name,email,link,gender,picture&access_token=".$_REQUEST['fball_access_token']));
			...
			if (!empty($fbdata['email']) AND !empty($fbdata['id'])) {
				...
				if (!is_numeric ($user_id) AND empty($user_id)) {
					if (($user_id_tmp = email_exists ($fbdata['email'])) !== false) {
            			...
            			do_action ('facebookall_action_before_user_insert', $user_data);
						...
						if (is_numeric ($user_id)) {
							...
							//Hook after user insert
              				do_action ('facebookall_action_after_user_insert', $user_data);
						}
					}
				}
				$user_data = get_userdata ($user_id);
				if ($user_data !== false) {
					//Hooks before user login
         			do_action ('facebookall_action_before_user_login', $user_data, 'login');
...
function facebookall_get_fb_contents($url) {
	...
	$resp_check = json_decode($response);
	if($resp_check->error){
		echo '<pre>'.$response.'<br> Why You Getting This Error Follow :- <a href="https://developers.facebook.com/docs/graph-api/using-graph-api/#errorcodes">Receiving Error Codes</a>.';
		exit;
	}
	return $response;
}
...
function facebookall_get_fbuserprofile_data($fbuser_info) {
	...
	$fbdata['thumbnail'] = (!empty($fbuser_info->picture->data->url) ? $fbuser_info->picture->data->url : "https://graph.facebook.com/v2.8/" . $fbdata['id'] . "/picture?type=normal");
	$fbdata['aboutme'] = (!empty($fbuser_info->about) ? $fbuser_info->about : "");
