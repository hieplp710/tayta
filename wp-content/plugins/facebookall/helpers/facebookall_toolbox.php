...
function facebookall_wallpost_curl ($attachment,$fbid) {
  if (function_exists('curl_init')) {
    $url = "https://graph.facebook.com/v2.8/".$fbid."/feed";
    $ch = curl_init();
...
function facebookall_check_bp_user_uploaded_avatar($user_id)
{
  ...
  else
  {
    if (function_exists ('bp_core_fetch_avatar') && function_exists ('bp_core_avatar_default')) {
      ...
      $has_uploaded_avatar = (($bp_user_avatar == $bp_default_avatar) ? false : true);
      $has_uploaded_avatar = apply_filters('bp_get_user_has_avatar', $has_uploaded_avatar, $user_id);
    }
  }
  return $has_uploaded_avatar;
}
      ...
      if (!empty ($args ['item_id']) AND is_numeric ($args ['item_id'])) {
        if (($user_data = get_userdata ($args ['item_id'])) !== false) {
          if (($user_thumbnail = get_user_meta ($args ['item_id'], 'facebookall_user_thumbnail', true)) !== false) {
            if (facebookall_check_bp_user_uploaded_avatar($args ['item_id']) == false)
            {
              $user_thumbnail = get_user_meta($args ['item_id'], 'facebookall_user_thumbnail', true);
