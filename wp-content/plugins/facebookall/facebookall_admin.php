...
function facebookall_getapp_result($apikey, $apisecret) {
  $url = "https://graph.facebook.com/v2.8/".$apikey."?access_token=".$apikey.'|'.$apisecret."&fields=id,name,website_url,app_domains,app_name,category,logo_url,link,daily_active_users,weekly_active_users,monthly_active_users";
...
function facebookall_save_admin_settings($fball_settings) {
  ...
  $fball_settings['comment_order_by'] = ((isset($fball_settings['comment_order_by']) && in_array($fball_settings['comment_order_by'], array('social', 'time', 'reverse_time'))) ? $fball_settings['comment_order_by'] : 'social');
  ...
  $fball_settings['follow_size'] = ((isset($fball_settings['follow_size']) && in_array($fball_settings['follow_size'], array('small', 'large'))) ? $fball_settings['follow_size'] : 'small');
  ...
  foreach(array('loginpage', 'registerpage', 'commentpage', 'comment_top', 'comment_bottom', 'comment_homepage', 'comment_posts', 'comment_pages', 'comment_postecerpts', 'comment_archives', 'comment_feed', 'share_facebooklike', 'share_facebook','share_twitter', 'share_gplus', 'share_top', 'share_bottom', 'share_home','share_posts', 'share_pages', 'share_postexcerpts', 'share_archives', 'share_feed') as $val){
          ...
          <div class="fballwelcome-panel-column-container">
            ...
            <div class="fballwelcome-panel-column">
              ...
              <?php if (!empty($fball_settings['apikey']) && !empty($fball_settings['apisecret'])) {
                ...
                $app_domain = (!empty($app_result->app_domains[0]) ? $app_result->app_domains[0] : $_SERVER['HTTP_HOST']);
                $app_site = (!empty($app_result->website_url) ? $app_result->website_url : site_url());
              ...
              <div style="float:left; width:100%;">
                <div>
                 ...
                 <div style="float: left; margin: 0 0 0 50px">
                   ...
                   <p style="margin:0 0 5px 0;"><?php _e('Site URL:', 'facebookall');?> <b><?php echo $app_site;?></b></p>
                   <p style="margin:0 0 5px 0;"><?php _e('Site Domain(s):', 'facebookall');?> <b><?php echo $app_domain;?></b></p>
            ...
            <td>
              <div class="fballrow fballrow_fbbutton">
                <div class="blank">
                 ...
                 <a href="javascript:void(0);" onclick="MakeApiRequest();"><b style="color:#FFFFFF !important;"><?php _e('Verify Connection Handler', 'facebookall');?></b></a>
               </div>
             </div>
           </td>
 ...
 <div id="tabs-2">
  ...
  <tr class="fballrow_white">
   ...
   <div id="tabs-3">
    ...
    <tr>
       <th><?php _e('Order By','facebookall');?><br /><small><?php _e('The order to use when displaying comments. ', 'facebookall');?></small></th>
       <td>
        <?php $social = "";$time = "";$reverse_time = "";
        if($fball_settings["comment_order_by"] == "social") $social = "checked='checked'";
        elseif($fball_settings["comment_order_by"] == "time") $time = "checked='checked'";
        elseif($fball_settings["comment_order_by"] == "reverse_time") $reverse_time = "checked='checked'";
        else $social = "checked='checked'";?>
        <input name="fball_settings[comment_order_by]" type="radio" <?php echo $social;?>value="social" />&nbsp;&nbsp;<?php _e('Social','facebookall');?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input name="fball_settings[comment_order_by]" type="radio" <?php echo $time;?>value="time" />&nbsp;&nbsp;<?php _e('Time','facebookall');?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input name="fball_settings[comment_order_by]" type="radio" <?php echo $reverse_time;?>value="reverse_time" />&nbsp;&nbsp;<?php _e('Reverse Time','facebookall');?>
      </td>
    </tr>
...
<div id="tabs-5">
  <table class="facebookall_table">
   ...
   <tr class="fballrow_white">
    <th scope="fballrow"><?php _e('URL to send','facebookall');?><br><small><?php _e('Leave blank for current url. ', 'facebookall');?></small></th>
...
<tr class="fballrow_white">
 ...
 <td>
   <?php $follow_small = "";
   $follow_large = "";
   if($fball_settings["follow_size"] == "small") $follow_small = "checked='checked'";
   elseif($fball_settings["follow_size"] == "large") $follow_large = "checked='checked'";
   else $follow_small = "checked='checked'";?>
   <input name="fball_settings[follow_size]" type="radio" <?php echo $follow_small;?>value="small" />&nbsp;&nbsp;<?php _e('Small','facebookall');?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <input name="fball_settings[follow_size]" type="radio" <?php echo $follow_large;?>value="large" />&nbsp;&nbsp;<?php _e('Large','facebookall');?>
 </td>
</tr>
