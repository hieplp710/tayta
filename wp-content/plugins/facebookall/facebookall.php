...
Description: Allow your visitors to <strong>comment, login, register and share with facebook, google and linkedin </strong> also post on their facebook wall.
Version: 1.8
Author: sourceaddons
...
function facebookall_front_scripts() {
  ...
  if ($fball_settings['share_twitter'] == '1') {
    wp_register_script('twitterjs', 'https://platform.twitter.com/widgets.js', false, '1.4.2');
...
function facebookall_add_meta_tags() {
    $fball_settings = get_option('fball_settings');
    if (!empty($fball_settings['apikey'])) {
        echo '<meta property="fb:app_id" content="'.$fball_settings['apikey'].'" />' . "\n";
    }
}
add_action( 'wp_head', 'facebookall_add_meta_tags' , 2 );

function facebookall_fbmlsetup() {
  $fball_settings = get_option('fball_settings');
  $appid = (!empty($fball_settings['apikey']) ? $fball_settings['apikey'] : '');
  ?>
<div id="fb-root"></div>
<script>(function(d, s, id) {
  ...
  js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.8&appId=<?php echo $appid;?>";
